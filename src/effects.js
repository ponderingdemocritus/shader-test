import { Effect, EffectComposer, RenderPass, EffectPass } from 'postprocessing'
    ...
    constructor() {
        ...
        /* glsl */ `
                  void mainImage(const in vec4 inputColor, const in vec2 uv, out vec4 outputColor) {
                      ...
                      if (G > threshold) {
                        outputColor = vec4(0, 0, 0, 1); // black for edges
                    } else {
                        outputColor = vec4(0, 0, 0, 0); // transparent for non-edges
                    }
                  }
              `,
        )
...
class CrossHatchEffect extends Effect {
    constructor() {
        super('CrossHatchEffect');
        this.threshold = 0.45;
    }

    render(gl, texture, width, height) {
        gl.bindFramebuffer(gl.FRAMEBUFFER, this.framebuffer);
        gl.clearColor(0, 0, 0, 0);
        gl.viewport(0, 0, width, height);

        // Set the uniforms.
        gl.uniform1i(this.uniforms.inputTexture, 0);
